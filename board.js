...
import cleanUpBoard from './gameFiles/cleanup';
...
function gameReducer(state = initialboard, action) {
	...
	switch (action.type) {
		...
		case 'Move':
			...
			state[action.tile1[0]][action.tile1[1]] =
				state[action.tile2[0]][action.tile2[1]];
...
export default function Board(props) {
	...
	function checkMove(x, y) {
		if (
			state[x][y] === state[tile1[0]][y - 1] &&
			state[x][y] === state[tile1[0]][y + 1]
		) {
			return true;
		} else if (
			state[x][y] === state[tile1[0]][y - 1] &&
			state[x][y] === state[tile1[0]][y - 2]
		) {
			return true;
		} else if (
			state[x][y] === state[tile1[0]][y + 1] &&
			...
			state[tile1[0]][tile1[1]] === state[x][y - 1] &&
			state[tile1[0]][tile1[1]] === state[x][y + 1]
		) {
			return true;
		} else if (
			state[tile1[0]][tile1[1]] === state[x][y - 1] &&
			state[tile1[0]][tile1[1]] === state[x][y - 2]
		) {
			return true;
		} else if (
			state[tile1[0]][tile1[1]] === state[x][y + 1] &&
			...
			state[x + 1] &&
			state[x - 1] &&
			state[tile1[0]][tile1[1]] === state[x - 1][y] &&
			state[tile1[0]][tile1[1]] === state[x + 1][y]
		) {
			return true;
		} else if (
			...
			state[x - 2] &&
			state[tile1[0]][tile1[1]] === state[x - 1][y] &&
			state[tile1[0]][tile1[1]] === state[x - 2][y]
		) {
			return true;
		} else if (
			state[x + 1] &&
			...
			state[tile1[0]][tile1[1]] === state[x][y - 3] &&
			state[tile1[0]][tile1[1]] === state[x][y - 2]
		) {
			return true;
		} else if (
			...
			state[x - 3] &&
			state[x - 2] &&
			state[tile1[0]][tile1[1]] === state[x - 3][y] &&
			state[tile1[0]][tile1[1]] === state[x - 2][y]
		) {
			return true;
		} else if (
